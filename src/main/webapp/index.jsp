		...
		<div class="col-md-12 mx-auto card-extended">
			...
			<div class="card mt-3 card-extended">
				...
				<div class="card-body">
					...
					<!-- El par?metro name es el que reconoce los servlets, en todo caso que el id sea igual al name -->
...
<script>
	function validateForm() {
		...
		if (!nameRegex.test(firstName.trim())) {
			alert("Los nombres s?lo pueden contener letras");
			return false;
		}

		if (!nameRegex.test(lastName.trim())) {
			alert("Los apellidos s?lo pueden contener letras");
			return false;
		}

		// Validamos los n?meros de DNI as? como los 8 digitos que contiene
		var dniRegex = /^\d{8}$/;

		if (!dniRegex.test(documentDNI.trim())) {
			alert("Debe ingresar los 8 n?meros de su DNI");
			return false;
		}
		...
		if (!celphoneRegex.test(celphone.trim())) {
			alert("Debe ingresar nueve d?gitos, empieza con 9");
			return false;
		}

		// Validate password format
		if (!isValidPassword(password)) {
			alert("S?lo permite 8 n?meros");
			return false;
		}
		...
		// Redirigir a una nueva p?gina JSP
